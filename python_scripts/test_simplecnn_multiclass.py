...
results_path = '/nas/home/cborrelli/cnn_bot/results/simple_cnn'
...
if __name__=='__main__':
    ...
    # Plot histories
    history_root = "/nas/home/cborrelli/cnn_bot/history/simple_cnn"
    history_filename = '/nas/home/cborrelli/cnn_bot/history/simple_cnn/model_classification_{}_classes_{}_lr_{}_balanced_{}.npy'.format(
    ...
    model_path = '/nas/home/cborrelli/cnn_bot/checkpoints/simple_cnn/model_classification_{}_classes_{}_lr_{}_balanced_{}'.format(
    classification_type, '_'.join(classes_list), learning_rate, balance_dataset)
    ...
    for c in tqdm.tqdm(classes_list, total=len(classes_list)):
        if traindev_eval :
            ...
            test_generator = SimpleCNNDataGenerator(dataframe=df_dev,
                                                 batch_size=batch_size,
                                                 shuffle=False,
                                                 classes_list=[c],
                                                 traindev_eval=traindev_eval,
            ...
            test_generator = SimpleCNNDataGenerator(dataframe=df_eval_test,
                                                batch_size=batch_size,
                                                classes_list=[c],
                                                classification_type=classification_type,
                                                traindev_eval=traindev_eval,
            ...
            dataset_train = 'eval_train'
            dataset_test = 'eval_test'
        ...
        test_rr = [[multiclass_dict[c], p, dataset_test ] for p in test_predicted]

        partial_results.extend(train_rr)
        partial_results.extend(test_rr)


    columns = ['class_true', 'class_pred', 'dataset']

    results = pd.DataFrame(columns=columns, data=partial_results)
    results.to_pickle(os.path.join(results_path, 'results_classification_{}_classes_{}_lr_{}_balanced_{}.pkl'.format(classification_type,
                                                                                               '_'.join(classes_list), learning_rate, balance_dataset) ))
